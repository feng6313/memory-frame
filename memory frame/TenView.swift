...
extension TenView {
    ...
    private func saveImageToPhotoLibrary() {
        ...
        // 使用ImageExporter保存图片
        ImageExporter.savePhotoFrameTen(
            image: selectedImage,
...
// MARK: - PhotoFrameViewTen
struct PhotoFrameViewTen: View {
    let image: UIImage
    ...
    // 上边框高度 = 背景宽度*0.16
    private var topBorderHeight: CGFloat {
        frameWidth * 0.16
    }
    
    // 左右边框宽度 = 背景宽度*0.32
    private var sideBorderWidth: CGFloat {
        frameWidth * 0.32
    }
        ...
        VStack(spacing: 0) {
            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    Rectangle()
                        .fill(Color.clear)
                        .frame(width: frameWidth, height: topBorderHeight)
                    
                    HStack(spacing: 0) {
                        // 左边框
                        Rectangle()
                            .fill(Color.clear)
                            .frame(width: sideBorderWidth, height: imageDisplayHeight)
                    ...
                    ZStack {
                        Rectangle()
                            .fill(Color.clear)
                            .frame(width: frameWidth, height: frameHeight - topBorderHeight - imageDisplayHeight)
                        
                        VStack {
                            // 文字和位置显示区域 - 位于背景高度*0.55处
                            VStack {
                                Spacer()
                                    .frame(height: frameHeight * 0.55 - topBorderHeight - imageDisplayHeight)
